      ...
      ? {
          ctl00$ContentPlaceHolder1$ddlMonth:
            moment()
              ...
              .month() + 1,
          ctl00$ContentPlaceHolder1$ddlYear: moment()
            .add(monthOffset, "months")
            .year(),
          ctl00$ContentPlaceHolder1$hidMonth:
            moment()
              .add(monthOffset, "months")
              .month() + 1,
          ctl00$ContentPlaceHolder1$hidYear: moment()
            .add(monthOffset, "months")
            .year()
        }
      : {};
    ...
    rp.post({
      ...
      form: Object.assign({}, ASPState, date, {
        ...
        ctl00$ScriptManager1:
          "ctl00$ScriptManager1|ctl00$ContentPlaceHolder1$btnsearch",
    ...
    for (var i = 1; i <= 42; i++) {
      ...
      if (date.length) {
        ...
        const remaining = parseInt(
          $(
            `#ContentPlaceHolder1_cc_${indexString} a span:nth-of-type(1)`
          ).text()
        );
        const waiting = parseInt(
          $(
            `#ContentPlaceHolder1_cc_${indexString} a span:nth-of-type(2)`
          ).text()
        );
        const applying = parseInt(
          $(
            `#ContentPlaceHolder1_cc_${indexString} a span:nth-of-type(3)`
          ).text()
        );
        beds.push({
          date: moment(`${parseInt(year) + 1911}-${month}-${day}`).format(
            "YYYY-MM-DD"
          ),
          remaining,
...
const getBeds = async name => {
  ...
  let nextMonth = await getBedStatus({
    ASPState: thisMonth.ASPState,
    name,
    monthOffset: 1
  });
