...
var parser = ($) => {
  ...
  for (var i = 1; i <= 42; i++) {
    ...
    if ($(gg).length) {
      ...
      capacityStatus.push({
        // 'date': moment().add(capacityStatus.length, 'day').format(),
        'remaining': remaining,
        'applying': waiting,
        'isDrawn': true,
      });
...
exports.crawl = (rooms, cb) => {
  var url = 'http://npm.cpami.gov.tw/bed_4.aspx'
  async.waterfall([
    (cb) => {
  ...
  (err, capacityStatus) => {
    if (err) {
      ...
      capacityStatus = capacityStatus.slice(7, 32).map( (c,i) => {
        c.date = moment().add(7+i, 'day').format();
        return c
      })
